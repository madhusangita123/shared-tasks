...
// imported below (e.g. `./joinSpaceByToken`, `./onTaskAssigned`) call
// `getFirestore()`/`getMessaging()` lazily, inside their request/event
// handlers — not at module load time — so the only real requirement is
// that this runs before any handler is ever invoked, which it always is
// (Cloud Functions runs this module top-to-bottom once per instance,
// before routing any request to a handler).
initializeApp();
